...
# This script is used for 454 Pyrosequencing (informed by: https://benjjneb.github.io/dada2/faq.html#can-i-use-dada2-with-my-454-or-ion-torrent-data)
# Also useful: https://forum.qiime2.org/t/cutadapt-not-trimming-454-reads/20247/4
...
# Path to dataset
DATASET_DIR <- "~/Repos/celiac-repository/16S_datasets/16S_25_Francavilla"
setwd(DATASET_DIR)
# Path to raw data
RAW_DATA_DIR <- "~/Repos/celiac-repository/16S_datasets/16S_25_Francavilla/fastqs"
...
TRUNC_LENGTH <- 460 # Optional and differs between datasets (!!!)
MAX_SEQUENCE_LENGTH <- 600 # Optional and differs between datasets (!!!)
filter_summary <- filterAndTrim(forward_reads, filtered_forward_reads,
                                truncLen=TRUNC_LENGTH, maxN=0, maxEE=2, truncQ=2, rm.phix=TRUE, 
                                compress=TRUE, multithread=FALSE, verbose=TRUE, maxLen = MAX_SEQUENCE_LENGTH)
...
dada_forward <- dada(filtered_forward_reads, err=error_rates_forward, multithread=TRUE, HOMOPOLYMER_GAP_PENALTY=-1, BAND_SIZE=32)
