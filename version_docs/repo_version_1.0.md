...
Lists all datasets included in this version, along with key information like publication details, SRA references, sample counts, country, and sample site(s). It contains the following columns:
  ...
  - `Multiple_Publications`: If a dataset is split across multiple publications it is noted here.
...
Contains metadata for every sample included in CMR v1.0. The file contains the following columns:
  ...
  - `Num_Reads_Nonchim`: The number of sequencing reads remaining after DADA2 processing (quality filtering, chimera removal, etc.). 'NA' for shotgun data.
  - `Percent_Host_Reads_Removed`: The percent of read pairs removed from the raw sequencing data by running Bowtie2 for host read removal.
  - `Total_Pairs_Pre_Host_Removal`: The number of read pairs in the raw sequencing data before running Bowtie2 for host read removal.
  - `Percent_Unclassified_Reads`: The percentage of reads that were unclassified by MetaPhlAn.
