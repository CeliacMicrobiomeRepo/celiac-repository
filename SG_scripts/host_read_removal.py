...
│   ├── host_read_removal.py         <-- This script
│   ├── tax_profiling.py
│   └── convert_metaphlan_gtdb.sh
├── SG_datasets/
│   ├── <Dataset_ID_1>/                 # e.g., SG_80_Mouzan
│   │   ├── fastqs/                     # Input trimmed FASTQs (paired)
│   │   │   ├── sampleA_1.fq.gz
│   │   │   ├── sampleA_2.fq.gz
│   │   │   └── ...
...
# Directory for the host genome index
host_genome/
...
- Ensure the `HOST_INDEX_PATH` points to the base name (e.g., `/path/to/host_genome/GRCh38_noalt_decoy_as`, not including `.1.bt2`).
...
HOST_INDEX_PATH = "/mnt/secondary/host_genome/GRCh38_noalt_decoy_as"
...
SKIP_SAMPLES_ALREADY_PROCESSED = True
...
def run_bowtie2(sample_name, r1_in, r2_in, host_index, out_dir, threads, sensitivity, bowtie2_exe):
    ...
    try:
        ...
        logging.error(f"Bowtie2 executable not found at 'bowtie2'. Please check configuration or system PATH.")
...
if __name__ == "__main__":
    ...
    # --- Validate Host Index ---
    # Make path handling more robust: if the given path is a directory,
    # assume the index files are inside it, using the directory's name as the base name.
    effective_host_index_path = HOST_INDEX_PATH
    if os.path.isdir(HOST_INDEX_PATH):
        logging.info(f"Host index path '{HOST_INDEX_PATH}' is a directory. Constructing index prefix from directory name.")
        effective_host_index_path = os.path.join(HOST_INDEX_PATH, os.path.basename(HOST_INDEX_PATH))

    logging.info(f"Using host index prefix: {effective_host_index_path}")
    logging.info(f"Log file: {LOG_FILE}")

    if not os.path.exists(effective_host_index_path + ".1.bt2"):
         logging.error(f"Host index file not found: {effective_host_index_path}.1.bt2")
         logging.error("Please ensure the Bowtie2 index files exist and the HOST_INDEX_PATH is set correctly.")
         sys.exit(1)


    logging.info(f"Found {len(DATASET_DIRS)} dataset directories to process:")
    ...
    for dataset_dir in DATASET_DIRS:
        ...
        output_fastq_dir = os.path.join(dataset_dir, OUT_FASTQS_SUBDIR)
        tsv_summary_path = os.path.join(dataset_dir, "about_host_read_removal.tsv")
        ...
        for sample_name, (r1_file, r2_file) in fastq_pairs.items():
            ...
            if SKIP_SAMPLES_ALREADY_PROCESSED:
                # Define expected output file paths
                output_prefix = os.path.join(output_fastq_dir, sample_name)
                expected_r1 = f"{output_prefix}{R1_PATTERN}.fq.gz"
                expected_r2 = f"{output_prefix}{R2_PATTERN}.fq.gz"
                expected_log = f"{output_prefix}_bowtie2.log"

                # If all expected output files are found, skip this sample
                if os.path.exists(expected_r1) and os.path.exists(expected_r2) and os.path.exists(expected_log):
                    logging.info(f"Skipping sample '{sample_name}' as output files already exist.")
                    continue
